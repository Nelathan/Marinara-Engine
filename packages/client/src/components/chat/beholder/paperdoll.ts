...
const DAMAGE_TIER: Record<string, any> = {
...
const SLOT_SIDE: Record<string, any> = {
  head: "center",
  face: "center",
  neck: "center",
  chest: "center",
...
const SLOT_Y: Record<string, any> = {
  head: 6,
  left_eye: 8,
  right_eye: 8,
  left_ear: 9,
...
const SLOT_LABEL: Record<string, any> = {
  head: "head",
  face: "face",
  neck: "neck",
  chest: "chest",
...
const FAMILY_LABEL_OVERRIDES: Record<string, any> = {
  digitigrade: {
    left_foot: "L. paw",
    right_foot: "R. paw",
  },
...
function labelOf(slot: any, family: any) {
...
const FAMILY_ALWAYS_SLOTS: Record<string, any> = {
...
export function setDollLayout(mode: any) {
...
function layoutSwitchHtml() {
  const opt = (mode: any, icon: any, title: any, label: any) =>
...
const SPECIES_FAMILIES: Record<string, any> = {
  humanoid: [
    "human",
    "elf",
    "half-elf",
...
export function familyOf(species: any) {
 ...
 */
function normalizeHolding(h: any) {
  if (!h) return null;
...
function escapeHtml(s: any) {
  ...
  return String(s ?? "").replace(/[&<>"']/g, (c: any) => ENTITIES[c] ?? c);
}

function damageMeta(d: any) {
 ...
 */
function computeSlotStates(state: any) {
  ...
  for (const [slot, sd] of Object.entries(body) as Array<[string, any]>) {
    // Per-slot flags:
...
function torsoState(perSlot: any, view: any) {
...
function partClasses(slotState: any) {
...
function partTitle(slotKey: any, slotState: any, family: any) {
...
function mirrorPath(d: any) {
  ...
  return d.replace(/([MLC])([^MLCZz]+)/g, (_: any, cmd: any, args: any) => {
    const nums = args
      .trim()
      .split(/[\s,]+/)
      .map(Number);
...
// pattern are for).
const FACE_FEATURES: Record<string, any> = {
...
function silhouetteSvg(perSlot: any, view: any, holding: any, family: any) {
  family = family || "humanoid";
  const part = (slotKey: any) => partClasses(perSlot[slotKey]);
  const title = (slotKey: any) => `<title>${escapeHtml(partTitle(slotKey, perSlot[slotKey], family))}</title>`;
  ...
  const faceFeature = (s: any, shape: any) => {
 ...
 */
function pelvisHtml(_perSlot: any, family: any, part: any, title: any) {
  let pelvis;
 ...
 */
function lowerBodyParts(perSlot: any, family: any, part: any, title: any, view: any) {
  view = view === "back" ? "back" : "front";
  const pathPart = (slot: any, d: any, extra?: any) =>
...
const HAND_POSITIONS: Record<string, any> = {
 ...
 */
function collectSlotRows(state: any) {
 ...
 */
export function renderCharacterDoll(name: any, state: any, view: any, opts = {}) {
  state = state || {};
  view = view === "back" ? "back" : "front";
  const placeholder = (opts as Record<string, any>).placeholder === true;
  const body = state.body || {};
  const family = familyOf(state.species);
  const offBodySlots = (OFF_BODY_SLOTS as Record<string, Set<string>>)[family] || OFF_BODY_SLOTS.humanoid;
  ...
  const cols: { left: any[]; right: any[] } = { left: [], right: [] };
  let figureSpan = "";
  if (paired) {
    ...
    for (const sc of slotCards) {
      ...
      if (mate && !bySlot.has(mate) && !offBodySlots.has(mate)) {
        bySlot.set(mate, { slot: mate, items: [] as any[], ghost: true } as any);
      }
    }
    const place = (entry: any, col: any, r: any) => {
  ...
  const multiSlot = (row: any) => {
    if (!row.slots || row.slots.length <= 1) return "";
    const list = row.slots.map((s: any) => labelOf(s, family)).join(", ");
  ...
  const renderSlotCard = ({ slot, items, style }: any) => {
    ...
    const wornItems = items.filter((i: any) => i.kind === "worn");
    const heldItems = items.filter((i: any) => i.kind === "holding");
    const ordered = [...wornItems, ...heldItems, ...items.filter((i: any) => i.kind === "wound")];
  ...
  const ghostCard = (slot: any, style: any) => {
    const styleAttr = style ? ` style="${style}"` : "";
  ...
  const renderColCard = (entry: any) => (entry.ghost ? ghostCard(entry.slot, entry.style) : renderSlotCard(entry));
  ...
  const digestHtml = isEmpty
    ? placeholder
      ...
      : ""
    : (() => {
        const slotLabelOf = (s: any) => labelOf(s, family);
        ...
        const wornBySlot = new Map<string, any>();
        ...
        for (const [slot, rows] of [...wornBySlot.entries()].sort(
          ...
          rows.forEach((row: any, i: any) => {
            (wornByRegion.get(region) as any[]).push({
              row,
              slot,
        ...
        const slotTag = (slot: any, layerIdx: any, layerTotal: any) => {
        ...
        const digestRow = ({ row, slot, layerIdx, layerTotal }: any) => `
            <li class="bh-digest-row">
                ${renderChip(row)}
                ${slotTag(slot, layerIdx || 0, layerTotal || 0)}
            </li>`;
        const section = (key: any, label: any, count: any, items: any) => {
        ...
        const woundItems = wounds.map(({ row, slot }: any) => digestRow({ row, slot, layerIdx: 0, layerTotal: 0 }));
        const heldItems = held.map(({ slot, item, damage }: any) => {
        ...
        const wornGroupedHtml =
          ...
          ((wornByRegion.get("other") as any[]).length
            ? `<li class="bh-digest-group">
                <h5 class="bh-digest-subhead">Other</h5>
                <ul class="bh-digest-group-list">${(wornByRegion.get("other") as any[]).map(digestRow).join("")}</ul>
            </li>`
            : "");
        const flagItems: any[] = [];
 ...
 */
export function renderDollPanel(state: any, activeName: any, updatedNames: any, view: any) {
