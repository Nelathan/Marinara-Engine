...
- Renaming, merging, and deleting tags across the library can now be undone from the confirmation toast. Marinara stores the previous tag list for each affected character, so a merge is reversible even though repeating it is not, and undo skips any character edited since.
- The Character Library gains an Organize view that proposes tag cleanup and organization from your own library, on your own machine: tags that differ only in punctuation or case, tags your most similar characters already use, and characters that look like versions of each other. Every suggestion names the evidence it came from, nothing is applied until you accept it, and no card content is sent to a provider.
- Tags can now be renamed or merged across the whole library from the tag explorer, with a confirmation that states how many characters change. Renaming a tag onto one that already exists is recognised as a merge and says so, and any active filter follows the tag to its new name.
- Characters can now be grouped into collections, and a character can belong to several at once. Collections appear as filters in the Character Library and selected characters can be added to a new or existing collection in one action.
- The Character Library now opens on shelves - All, Used, Never used, Needs review, Try later, and Archived - each showing how many characters are waiting on it, and characters can be given a library status in bulk from the selection bar. Status is stored locally, so archiving a character never changes the character card or its exports.
- The character editor's tag field now suggests tags already used in the library, with usage counts, and reuses their existing spelling so a new card does not introduce another variant of a tag that already exists.
