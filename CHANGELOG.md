...
- Tactical combat's tile inspector no longer blocks battlefield tiles or party-unit selection; its header remains draggable and closable. Restart and Retry now wait for session hydration and recover a replacement battle after a lost response instead of leaving a stale combat locked on screen.
- Combat mode now clears terminal snapshots before the next battle can restore them, binds Tactical sessions to their declaring turn, and waits for authoritative Classic/Tactical session hydration so sequential fights cannot replay an earlier outcome.
