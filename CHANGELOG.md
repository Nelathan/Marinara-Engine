...
- Continuing past a Tactical defeat or victory no longer relaunches a fresh battle against the finished encounter while the GM's aftermath narration is still generating: the aftermath now leaves combat server-side immediately, combat hydration stands down during the handoff, and no automatic launch path can replace a completed session that owns the current combat declaration. Completing a session also stamps the cached active-session lookups as terminal, so the stale-unwind self-heal can no longer re-adopt the finished battle and wedge the chat on its outcome screen.
- Fleeing during an unfinished escape objective is now rejected in Classic and Tactical combat until the party reaches the exit. Ordinary fights with no escape objective still retreat immediately, and a completed escape still ends as a successful getaway.
- Classic combat now reuses the in-flight action id when the same command is clicked twice before the first response lands, so the server's duplicate-action replay can prevent a second turn from resolving.
