...
No próprio computador (`127.0.0.1`), a senha nunca é necessária. Os outros dispositivos ficam bloqueados até você configurar o controle de acesso (Basic Auth ou lista de IPs permitidos). Cada opção está explicada em [Acesso remoto](REMOTE_ACCESS.md).
...
O código de navegador roda em um Worker dedicado, dentro de um iframe de origem opaca. O código de servidor roda em um processo separado, isolado pelo sistema operacional, em hosts macOS e Linux compatíveis. Os dois ambientes recebem apenas permissões limitadas de log, de armazenamento privado, de temporizador e de limpeza.

As importações de terceiros ficam ocultas por padrão. O operador do host precisa definir `ENABLE_EXTERNAL_EXTENSIONS=true` no arquivo `.env`, e depois o usuário precisa aceitar o aviso em **Settings → Advanced → Danger Zone**. Enquanto essas duas travas não forem abertas, os registros externos, inclusive os guardados manualmente e os que vieram de importação de perfil, não aparecem, não podem ser aprovados e não podem ser executados. Veja [Extensões pessoais](extending/personal-extensions.md).
...
## Como funcionam as animações de storyboard do Game Mode?

Um **storyboard** transforma um turno de narração já concluído do game master (o mestre do jogo) em uma sequência curta de quadros-chave no estilo mangá. Ele também pode acrescentar clipes animados curtos. O turno então é reproduzido como uma minicena de corte. Os storyboards existem só no **Game Mode**.

Para criar um manualmente, abra a **Gallery** (galeria) e clique em **Create storyboard** no turno de narração mais recente. Para reabrir um storyboard que você fechou, clique em **View storyboard**.

Para criá-los automaticamente, abra **Chat Settings**, vá em **Agents**, encontre o card **Storyboards** e ative **Automatic Storyboard Illustrations**. Ative também **Automatic Storyboard Animations** se quiser os clipes de vídeo, o que exige uma conexão de geração de vídeos. O fluxo completo está no [Guia do Storyboard Engine](game/storyboard.md).
...
Sim. Adicione uma conexão de geração de imagens, por exemplo **Pollinations** (dispensa chave) ou um provedor pago. Com isso, Marinara cria avatares de personagem, arte de cena, selfies e storyboards do Game Mode. Para adicionar uma conexão, veja [Conectando a um provedor de IA](connections/connecting-to-a-provider.md).
