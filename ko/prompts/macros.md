...
| `{{addvar::name::value}}` | 저장한 값 끝에 텍스트를 덧붙입니다. |
| `{{incvar::name}}` | 숫자 변수에 1을 더합니다. |
| `{{decvar::name}}` | 숫자 변수에서 1을 뺍니다. |

변수는 한 번의 프롬프트 생성 안에서 왼쪽부터 오른쪽으로 치환됩니다. 앞쪽에서 설정한 값, 예컨대 먼저 오는 로어북 항목에서 설정한 값은 같은 프롬프트의 뒤쪽에서 읽을 수 있습니다.

범위와 관련해 꼭 알아 둘 제약이 있습니다. 이 변수들은 답변 하나 동안만 살아 있습니다. Marinara는 값을 어디에도 저장하지 않습니다. 다음 답변을 생성할 때는 모든 변수가 다시 빈 상태로 시작합니다. `{{setvar}}`가 턴을 넘겨 값을 기억할 것이라고 기대하지 마세요.
...
- 변수가 유지될 것이라고 기대하지 마세요. `{{setvar}}`로 설정한 값은 다음 답변에서 초기화됩니다.
