...
- Utworzenie lub edycja motywu, agenta, presetu promptu albo szkicu rozszerzenia Personal Extension. Professor Mari jest jedyną domyślną autorką rozszerzeń. Jej szkice pozostają wyłączone, dopóki nie sprawdzisz kodu w piaskownicy i nie zatwierdzisz jego dokładnego skrótu w **Settings** (Ustawienia) > **Addons**.
- Porównanie wszystkich 29 oficjalnych agentów i pakietów funkcji do pobrania, wyjaśnienie, które tryby obsługują, i podpowiedź, które pasują do celu użytkownika. Asystentka odróżnia dostępność w katalogu od tego, co jest naprawdę zainstalowane, w razie potrzeby kieruje do sekcji **Agents → Download Agents** i wie, że źródła pakietów oraz pełny katalog znajdują się w repozytorium [Pasta-Devs/Marinara-Agents](https://github.com/Pasta-Devs/Marinara-Agents).
