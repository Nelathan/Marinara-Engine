...
Kod przeglądarkowy działa w osobnym wątku Worker wewnątrz ramki iframe o nieprzejrzystym pochodzeniu. Kod serwerowy działa w oddzielnym procesie izolowanym przez system, na obsługiwanych hostach macOS i Linux. Oba środowiska dostają tylko wąskie uprawnienia: logowanie, prywatny magazyn danych, liczniki czasu i sprzątanie.

Import z zewnątrz jest domyślnie ukryty. Operator hosta musi ustawić `ENABLE_EXTERNAL_EXTENSIONS=true` w pliku `.env`, a potem użytkownik musi zaakceptować ostrzeżenie w sekcji **Settings → Advanced → Danger Zone**. Dopóki obie bramki nie zostaną otwarte, rekordy zewnętrzne, w tym zapisane ręcznie i zaimportowane z profilu, nie są widoczne, nie da się ich zatwierdzić ani uruchomić. Zobacz [Rozszerzenia osobiste](extending/personal-extensions.md).
