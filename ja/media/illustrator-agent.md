...
Illustratorはすべてのメッセージを絵にするわけではありません。デフォルトでは、1枚生成したあと、採用されたAIの返信がおよそ5回進むまで次の生成を行いません。同じ返信をスワイプしたり再生成したりしても、この間隔は進みません。絵にする場面ではないと判断したときは、その返信を飛ばして画像を作りません。生成した画像はすべてチャットの**Gallery**(ギャラリー)に保存します。
