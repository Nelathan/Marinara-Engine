...
Sur le même ordinateur (`127.0.0.1`), aucun mot de passe n'est jamais nécessaire. Les autres appareils restent bloqués tant que le contrôle d'accès n'est pas configuré (Basic Auth ou liste d'autorisation d'adresses IP). Chaque option est expliquée dans [Accès à distance](REMOTE_ACCESS.md).
...
Sous Android, tu peux aussi installer un APK, le fichier d'installation des applications Android. Il fait tourner Marinara localement sur le téléphone. Voir [Installation sur Android](installation/android-termux.md). Sur iPhone et iPad, voir le [guide PWA pour iOS](installation/ios-pwa.md).
...
Le code navigateur s'exécute dans un Worker dédié, à l'intérieur d'une iframe à origine opaque. Le code serveur s'exécute dans un processus séparé, isolé par le système, sur les hôtes macOS et Linux pris en charge. Les deux environnements ne reçoivent que des capacités restreintes : logs, stockage privé, minuteries et nettoyage.

Les imports tiers sont masqués par défaut. L'opérateur de l'hôte doit définir `ENABLE_EXTERNAL_EXTENSIONS=true` dans le fichier `.env`, puis l'utilisateur doit accepter l'avertissement sous **Settings → Advanced → Danger Zone**. Tant que ces deux verrous ne sont pas levés, les enregistrements externes, y compris ceux stockés manuellement et ceux importés depuis un profil, n'apparaissent pas, ne peuvent pas être approuvés et ne peuvent pas s'exécuter. Voir [Extensions personnelles](extending/personal-extensions.md).
...
## Comment fonctionnent les animations de storyboard en Game Mode ?

Un **storyboard** transforme un tour de narration terminé du Game Master en une courte séquence d'images-clés façon manga. Il peut aussi ajouter de brefs clips animés. Le tour se rejoue alors comme une mini-cinématique. Les storyboards n'existent qu'en **Game Mode**.

Pour en créer un à la main, ouvre la **Gallery** (galerie) et clique sur **Create storyboard** pour le dernier tour de narration. Pour rouvrir un storyboard que tu as fermé, clique sur **View storyboard**.

Pour les générer automatiquement, ouvre **Chat Settings**, va dans **Agents**, trouve la carte **Storyboards** et active **Automatic Storyboard Illustrations**. Active aussi **Automatic Storyboard Animations** si tu veux également des clips vidéo : cela demande une connexion de génération de vidéos. Pour le déroulé complet, voir [Storyboards en Game Mode](game/storyboard.md).
