...
如果想对这些客户端执行常规的 Basic Auth 和 IP 允许列表检查，请把相应开关设为 `false`。自动检测不可用时，设为 `true` 可保留旧版的宽泛放行：Tailscale 会信任整个 `100.64.0.0/10`，Docker 还会信任检测到的接口和网关以及旧版 `172.16.0.0/12` 网段。只有在所有匹配对端都可信时才使用这种兼容模式。
