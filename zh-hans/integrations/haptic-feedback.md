...
触感反馈是聊天 **Agents**(智能体) 之一，也就是可以加进聊天的那些 AI 帮手。它在 Conversation(对话模式) 和 Roleplay(角色扮演) 下都能用，Game Mode(游戏模式) 里没有。
...
1. 打开一个 Conversation 或 Roleplay 聊天。Game Mode 里没有触感反馈这个选项。
...
在 Roleplay 聊天里，**Haptic Feedback** 卡片会显示一个 **Touch sensitivity**(触感强度) 控件，有三个档位，旁边有一行小字“Roleplay only”。这个控件只对 Roleplay 聊天生效。其他模式下强度设置会被忽略，触碰指令也不受这几档的限制。

三个档位决定每次触碰指令的强度上限和持续时长。

| 档位 | 手感 | 说明 |
|---|---|---|
| **Subtle** | 强度更低，反馈更短 | 最轻柔的一档 |
| **Standard** | 适合大多数场景的均衡反馈 | 默认档位 |
| **Intense** | 反馈更强，上限更高 | 最强的一档 |

默认选中 **Standard**。挑一个和当前场景相称的档位就行。在 Roleplay 聊天里，Marinara 会把每一次触碰指令都限制在所选档位的范围内，AI 无法突破。

## 无意触碰

在强度控件下方，Roleplay 聊天还会显示一个 **Incidental contact**(无意触碰) 开关，说明文字是“Tiny taps for accidental brushes and bumps.”这个开关默认是关的。

关闭时，AI 会忽略故事里那些轻微的无意触碰，只为刻意的或有力度的接触发出指令。想让轻蹭和碰撞也带来轻微震动，就把它开启。和触感强度一样，这个控件也只出现在 Roleplay 聊天里。
